...
app.get('/api/videos', (req, res) => {
    const data = getData();
    res.json({ success: true, count: data.videos.length, data: data.videos.map(v => ({ ...v, _id: v.id })) });
});
...
app.get('/api/health', (req, res) => {
    res.json({ status: 'OK', message: 'Server is running (Serverless mode)', timestamp: new Date().toISOString() });
});

app.get('/api', (req, res) => {
    res.json({ message: 'Faculty E-Content API', version: '1.0.0', mode: 'Serverless' });
});
